-- Kullanici yonetim fonksiyonlari duzeltmesi (canli DB'ye SQL Editor ile uygulandi).
-- Sorunlar:
--   1) crypt()/gen_salt() pgcrypto fonksiyonlari 'extensions' semasindadir;
--      search_path'te olmadigi icin 'function gen_salt(unknown) does not exist'
--      hatasi veriyordu. Tam nitelikli extensions.crypt()/gen_salt() kullanildi.
--   2) auth.users.confirmed_at kolonu yeni Supabase semasinda GENERATED kolondur;
--      elle insert edilemez ('cannot insert a non-DEFAULT value'). Insert'ten cikarildi.
