  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Yeni Kullanıcı</h1>
        <button
          onClick={onIptal}
          style={{
            padding: '8px 16px', backgroundColor: 'var(--bg-subtle)', border: 'none',
            borderRadius: 10, fontSize: 13, fontWeight: 600, color: 'var(--text-muted)', cursor: 'pointer',
          }}
        >
          İptal
        </button>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>
            <label style={labelStyle}>
              Giriş E-postası <span style={{ color: 'var(--text-subtle)' }}>(otomatik)</span>
            </label>
            <div style={{ fontSize: 13, color: 'var(--text-faint)', backgroundColor: 'var(--bg-hover)', padding: '10px 12px', borderRadius: 10 }}>
              {adSoyad.trim() ? kullaniciEpostasi(adSoyad.trim()) : '—'}
            </div>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 4 }}>
            <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>Yönetici (Şantiye Şefi yetkisi)</span>
            <input type="checkbox" checked={admin} onChange={(e) => setAdmin(e.target.checked)} style={toggleStyle} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>Proje Müdürü</span>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {adalar.map((a) => {
                  ...
                  return (
                    <label key={a.ada} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-muted)', cursor: 'pointer' }}>
        ...
        {hata && (
          <div style={{ marginTop: 12, fontSize: 12, color: '#ef4444', backgroundColor: 'var(--bg-danger)', padding: '8px 12px', borderRadius: 8 }}>
            {hata}
          </div>
        )}
        {olusturulanEposta && (
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: kaydediliyor ? 'var(--border)' : '#f59e0b',
            border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 600,
            color: kaydediliyor ? 'var(--text-subtle)' : '#fff',
  ...
  return (
    <div style={{ ...card, marginBottom: 16 }}>
      <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)', margin: 0, marginBottom: 12 }}>
        Yetki ve Rol
        {kendim && <span style={{ fontSize: 11, color: 'var(--text-subtle)', fontWeight: 400 }}> (kendi kaydınız)</span>}
      </h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label style={labelStyle}>Rol</label>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>Yönetici (Şantiye Şefi)</span>
          <input
            type="checkbox"
            checked={admin}
            disabled={kendim}
        ...
        <button
          ...
          disabled={kaydediliyor}
          style={{
            width: '100%', padding: 11, backgroundColor: kaydediliyor ? 'var(--border)' : '#f59e0b',
        ...
        {!kendim && (
          <button
            onClick={sil}
            style={{
              width: '100%', padding: 11, backgroundColor: 'var(--bg-danger)',
              border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600,
